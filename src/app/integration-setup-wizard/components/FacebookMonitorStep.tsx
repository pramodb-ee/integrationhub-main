...
import { AlertConfigModal } from './MonitorBootstrapStep';
...
import {
  ...
  Clock, Download, ExternalLink, Loader2, Plus, RefreshCw, Search, TrendingUp, X, XCircle, Zap,
} from 'lucide-react';

type LeadSyncStatus = 'success' | 'failed';

interface FacebookLeadLog {
  id: string;
  name: string;
  email: string;
  phone: string;
...
const LEAD_FORMS = ['Contact Us Form', 'Free Trial Signup', 'Newsletter Subscribe', 'Demo Request Form'];
...
const FAILURE_REASONS = [
  ...
  'Field mapping validation failed for Campaign Name',
];

const LEAD_SEEDS: { name: string; email: string; phone: string; form: string; receivedAt: string; status: LeadSyncStatus; failureReasonIndex?: number }[] = [
  { name: 'Ananya Krishnan', email: 'ananya.krishnan@gmail.com', phone: '+91 98765 43210', form: 'Contact Us Form', receivedAt: '09:52:18', status: 'success' },
  { name: 'Rohit Bansal', email: '', phone: '+91 90040 55211', form: 'Free Trial Signup', receivedAt: '09:48:02', status: 'failed', failureReasonIndex: 0 },
  { name: 'Priya Menon', email: 'priya.menon@outlook.com', phone: '+91 97170 88342', form: 'Demo Request Form', receivedAt: '09:45:41', status: 'success' },
  { name: 'Karthik Iyer', email: 'karthik.iyer@bizmail.com', phone: '+91 99010 22114', form: 'Contact Us Form', receivedAt: '09:41:15', status: 'success' },
  { name: 'Fatima Sheikh', email: 'fatima.sheikh@gmail.com', phone: '+91 98220 77654', form: 'Newsletter Subscribe', receivedAt: '09:38:22', status: 'failed', failureReasonIndex: 2 },
  { name: 'Rohan Deshmukh', email: 'rohan.d@corpmail.com', phone: '+91 96650 33298', form: 'Free Trial Signup', receivedAt: '09:35:44', status: 'success' },
  { name: 'Neha Gupta', email: 'neha.gupta@gmail.com', phone: 'invalid-number', form: 'Demo Request Form', receivedAt: '09:32:05', status: 'failed', failureReasonIndex: 3 },
  { name: 'Arjun Nair', email: 'arjun.nair@gmail.com', phone: '+91 90350 44112', form: 'Contact Us Form', receivedAt: '09:29:18', status: 'success' },
  { name: 'Simran Kaur', email: 'simran.kaur@gmail.com', phone: '+91 98980 66771', form: 'Free Trial Signup', receivedAt: '09:26:02', status: 'success' },
  { name: 'Deepak Verma', email: 'deepak.verma@corpmail.com', phone: '+91 99870 11223', form: 'Newsletter Subscribe', receivedAt: '09:23:40', status: 'success' },
  { name: 'Ritu Chawla', email: 'ritu.chawla@gmail.com', phone: '+91 98110 44556', form: 'Contact Us Form', receivedAt: '09:20:58', status: 'success' },
  { name: 'Manoj Pillai', email: '', phone: '+91 90080 22337', form: 'Demo Request Form', receivedAt: '09:17:20', status: 'failed', failureReasonIndex: 0 },
  { name: 'Sneha Kulkarni', email: 'sneha.k@gmail.com', phone: '+91 97640 33221', form: 'Free Trial Signup', receivedAt: '09:14:05', status: 'success' },
  { name: 'Aditya Bhatt', email: 'aditya.bhatt@bizmail.com', phone: '+91 96330 55009', form: 'Contact Us Form', receivedAt: '09:10:47', status: 'success' },
  { name: 'Pooja Reddy', email: 'pooja.reddy@gmail.com', phone: '+91 90420 66118', form: 'Newsletter Subscribe', receivedAt: '09:07:12', status: 'failed', failureReasonIndex: 4 },
  { name: 'Suresh Kumar', email: 'suresh.kumar@gmail.com', phone: '+91 99230 55142', form: 'Demo Request Form', receivedAt: '09:04:03', status: 'success' },
  { name: 'Divya Krishnan', email: 'divya.k@outlook.com', phone: '+91 98760 12987', form: 'Contact Us Form', receivedAt: '09:00:55', status: 'success' },
  { name: 'Amitabh Joshi', email: 'amitabh.joshi@corpmail.com', phone: '+91 99500 44778', form: 'Free Trial Signup', receivedAt: '08:57:38', status: 'success' },
];

const MOCK_LEADS: FacebookLeadLog[] = LEAD_SEEDS.map((seed, index) => ({
  id: `fb-lead-${1001 + index}`,
  name: seed.name,
  email: seed.email,
  phone: seed.phone,
  form: seed.form,
...
interface FacebookMonitorStepProps {
  integrationName?: string;
}

export default function FacebookMonitorStep({ integrationName = 'Facebook Lead Ads Integration' }: FacebookMonitorStepProps) {
  const [alertModalOpen, setAlertModalOpen] = useState(false);
  const [catalogOpen, setCatalogOpen] = useState(false);
  const [secondsAgo, setSecondsAgo] = useState(0);
  const [refreshing, setRefreshing] = useState(false);

  const [leads, setLeads] = useState<FacebookLeadLog[]>(MOCK_LEADS);
  ...
  const kpis = [
    ...
    { id: 'k-latency', label: 'Avg Sync Time', value: '312ms', icon: Clock, iconBg: '#FFFBEB', iconColor: '#D97706' },
  ];
  ...
  const filteredLeads = useMemo(() => leads.filter((lead) => {
    ...
    const matchSearch = !q || lead.name.toLowerCase().includes(q) || lead.email.toLowerCase().includes(q);
    const matchStatus = statusFilter === 'all' || lead.status === statusFilter;
    const matchForm = formFilter === 'all' || lead.form === formFilter;
    return matchSearch && matchStatus && matchForm;
  }), [leads, search, statusFilter, formFilter]);

  const PAGE_SIZE = 6;
  const [page, setPage] = useState(1);
  useEffect(() => { setPage(1); }, [search, statusFilter, formFilter]);
  ...
  return (
    <div className="space-y-5">
      <AlertConfigModal open={alertModalOpen} onClose={() => setAlertModalOpen(false)} integrationName={integrationName} />
      <IntegrationCatalogModal open={catalogOpen} onClose={() => setCatalogOpen(false)} />

      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          <ConnectorIcon type={'facebook' as ConnectorType} size={40} />
          <div>
            ...
            {leads.filter((l) => l.status === 'failed').slice(0, 3).map((lead) => (
              <div key={lead.id} className="flex items-start gap-2 p-2.5 rounded-lg bg-danger-bg/60 border border-danger-border">
                ...
                <div className="min-w-0 flex-1">
                  <p className="text-[11px] font-semibold text-foreground truncate">{lead.name} — {lead.form}</p>
      ...
      <div className="card-base overflow-hidden">
        <div className="p-4 border-b border-border">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
            <div>
              ...
              <p className="text-[11px] text-muted-foreground mt-0.5">Every lead received from Facebook Lead Ads — review sync status and failure reasons.</p>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="relative">
              <select value={formFilter} onChange={(e) => setFormFilter(e.target.value)} className="h-8 pl-3 pr-7 text-[12px] bg-muted rounded-md border border-border appearance-none focus:outline-none focus:ring-1 focus:ring-primary">
                <option value="all">All Forms</option>
                {LEAD_FORMS.map((form) => <option key={form} value={form}>{form}</option>)}
              </select>
              <ChevronDown size={11} className="absolute right-2 top-1/2 -translate-y-1/2 text-muted-foreground pointer-events-none" />
            </div>
            {(search || statusFilter !== 'all' || formFilter !== 'all') && (
              <button onClick={() => { setSearch(''); setStatusFilter('all'); setFormFilter('all'); }} className="flex items-center gap-1 text-[11px] text-muted-foreground hover:text-foreground">
        ...
        <div className="overflow-x-auto">
          <div className="min-w-[900px]">
            <div className="grid grid-cols-[100px_1.2fr_1fr_0.9fr_1.6fr_100px] gap-3 px-4 py-2.5 bg-muted/40 border-b border-border items-center">
              ...
              <span className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">Form</span>
            ...
            <div className="divide-y divide-border">
              {paginatedLeads.map((lead) => {
                ...
                return (
                  <div key={lead.id} className="grid grid-cols-[100px_1.2fr_1fr_0.9fr_1.6fr_100px] gap-3 px-4 py-3 items-center hover:bg-muted/30 transition-colors">
                    ...
                    <span className="text-[12px] text-muted-foreground truncate">{lead.form}</span>
